...
const FindList = () => {
  ...
  useEffect(() => {
    fetch(`http://10.58.4.88:8000/findstay`, {
      method: 'Get',
    })
      .then(res => res.json())
      .then(result => {
  ...
  useEffect(() => {
    fetch(`http://10.58.4.88:8000/findstay?offset=${offset}`, {
      method: 'Get',
    })
      .then(res => res.json())
      .then(result => {
  ...
  const handleAllPeople = () => {
    ...
    fetch(
      `http://10.58.4.88:8000/findstay?${checkTavelInputText}${checkDate}${checkSort}${checkAreaData}${checkAllPeople}${checkTavelValue}${checkTavelStayText}${checkTavelThemeText}`,
      {
        method: 'Get',
      }
    )

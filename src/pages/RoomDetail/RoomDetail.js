...
const RoomDetail = () => {
  ...
  useEffect(() => {
    fetch(`http://10.58.4.88:8000/room/${params.id}`)
      .then(res => res.json())
      .then(res => {
        setOptionDataList({
          ...optionDataList,
          room_id: res.result[0].room_id,
  ...
  const bookRoom = () => {
    ...
    const endMonth = end && ('0' + (end.getMonth() + 1)).slice(-2);
    ...
    fetch(`http://10.58.4.88:8000/books`, {
      method: 'POST',
      headers: {
        Authorization: token,
      },
      body: JSON.stringify({
        room_id: optionDataList.room_id,
  ...
  return (
    <RoomDetailContainer>
      ...
      <BookButtonBottom onClick={bookRoom}>결제하기</BookButtonBottom>
    </RoomDetailContainer>
  );
};
...
const BookButtonBottom = styled.button`
  margin: 100px 0;
  width: 250px;
  height: 50px;
  border: 1px solid #000;
